...
use crate::protocol::command::{
    CommandFrame, DiscoveryAck, build_param_config, build_param_inquire, host_ip_cfg_value,
    parse_param_config_ack, parse_param_inquire_ack,
};
use crate::protocol::{
    CmdId, DataType, HOST_CMD_PORT, HOST_DATA_PORT, HOST_IMU_PORT, ParameterKey, ReturnCode,
};
...
impl LivoxClient {
    ...
    pub async fn discover(
        ...
        req.write(&mut buf)?;
        self.cmd_socket
            .send_to(&buf[..req.length as usize], broadcast_addr)
            .await?;
        ...
        if let Some(directed) = directed_broadcast_for(local_addr.ip().to_string().parse().ok()) {
            ...
            if directed_addr != broadcast_addr {
                self.cmd_socket
                    .send_to(&buf[..req.length as usize], directed_addr)
                    .await?;
            }
        }
    ...
    pub async fn send_command(
        ...
        self.cmd_socket
            .send_to(&send_buf[..req.length as usize], addr)
            .await?;
    ...
    pub async fn set_work_mode(
        ...
        self.set_parameter(
            addr,
            &[(ParameterKey::WorkTgtMode, vec![state as u8])],
            wait,
        )
        .await
    }
    ...
    pub async fn set_data_type(
        ...
        self.set_parameter(
            addr,
            &[(ParameterKey::PclDataType, vec![data_type as u8])],
            wait,
        )
        .await
    }
    ...
    pub async fn start_streaming(
        ...
        self.set_stream_destinations(addr, data_dst, imu_dst, wait)
            .await?;
...
mod tests {
    ...
    use crate::protocol::{CmdType, SenderType};
    ...
    async fn discover_mock_lidar() {
        ...
        tokio::spawn(async move {
            ...
            mock.send_to(&out[..ack.length as usize], from)
                .await
                .unwrap();
        });

        let devices = client
            .discover(mock_addr, Duration::from_secs(1))
            .await
            .unwrap();
